...
class ConductorController {
    ...
    register(req, res) {
        conductor.create(req.body, (error, data) => {
            if (error) {
                res.status(500).send();
            } else {
    ...
    update(req, res) {
        ...
        conductor.findByIdAndUpdate(id, { $set: obj }, (error, data) => {
            if (error) {
                res.status(500).send();
            } else {
    ...
    deleteDrivers(req, res) {
        let { id } = req.body;
        conductor.findByIdAndDelete(id, (error, data) => {
            if (error) {
                res.status(500).send();
            } else {
                res.status(200).json(data);
    ...
    getDrivers(req, res) {
        let id = req.params.id;
        conductor.findById(id, (error, data) => {
            if (error) {
                res.status(500).send();
            } else {
                res.status(200).json(data);
